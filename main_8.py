...
class SimplifiedInternshipMatcher:
    ...
    def create_d3_force_mindmap_html(self, result, job_title: str):
        """
        Create a D3.js force-directed mind map with physics simulation
        """
        ...
        for i, category_match in enumerate(categories):
            cat_id = f'cat_{i}'
            has_matches = bool(category_match.parent_skill_groups or category_match.standalone_skills)
            ...
            for parent, children in category_match.parent_skill_groups.items():
                ...
                for child in children[:2]:
                    ...
                    links.append({'source': skill_id, 'target': child_id, 'value': 1})
                    node_id += 1
        ...
        <body>
            ...
            <script>
                ...
                const color = d3.scaleOrdinal()
                    .domain([0, 1, 2, 3, 4, 5])
                    .range(['#2E86AB', '#28A745', '#FFC107', '#17A2B8', '#6F42C1', '#DC3545']);
                ...
                node.append("circle")
                    .attr("r", d => {{
                        ...
                        if (d.type.includes('skill')) return 20;
                        return 15;
                    }})
                    .attr("fill", d => {{
                        if (d.type === 'center') return '#2E86AB';
                        if (d.type === 'category') return d.satisfied ? '#28A745' : '#DC3545';
                        if (d.type === 'parent_skill') return '#FFC107';
                        if (d.type === 'standalone_skill') return '#17A2B8';
                        return '#6F42C1';
                    }})
                    .attr("stroke", "white")
                    .attr("stroke-width", 3);
                ...
                node.on("mouseover", function(event, d) {{
                    let tooltipText = `${{d.name}}<br>Type: ${{d.type.replace('_', ' ')}}`;
                    if (d.children) {{
                        tooltipText += `<br>Children: ${{d.children.join(', ')}}`;
                    }}
    ...
    def create_category_skill_matches(self, matched_skills_by_category: Dict[str, List[str]]) -> List[CategorySkillMatch]:
        """
        Fallback method for basic category skill matching.
        """
        category_matches = []
        ...
        for category_name, matched_skills in matched_skills_by_category.items():
            ...
            # Basic grouping by immediate parent
            for skill_name in matched_skills:
                ...
                if immediate_parent:
                    ...
                    parent_groups[immediate_parent].append(skill_name)
...
# ---- Streamlit App ----
st.set_page_config(page_title="Enhanced Student-Job Matcher with Mind Map", layout="wide")
st.title("🎓 Enhanced Student-Job Matching with Interactive Mind Map 🧠")
...
if ready_to_run:
    st.header("Matching Results with Interactive Mind Map Visualization")
    ...
    if selected_email:
        ...
        st.subheader(f"🧠 Professional Skills Mind Map: {selected_role_name}")

        # Try Enhanced Plotly first (best option)
        d3_html = matcher.create_d3_force_mindmap_html(result, selected_role_name)
    
        import streamlit.components.v1 as components
        components.html(d3_html, height=720, width=720,scrolling=False)
        
        st.success("🎯 D3 mind map loaded! Drag nodes and use controls.")
    # Export
    ...
    st.download_button(
        ...
        data=csv_content,
        file_name=f"enhanced_mind_map_student_job_matching_{time.strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv"
    )
...
with st.expander("📋 Instructions & Key Fixes"):
    ...
    **🔧 KEY FEATURES:**
    ...
    1. **🧠 Interactive Mind Map Visualization**: Visual representation of skill relationships and hierarchies
    2. **Fixed Skill Taxonomy Loading**: Now properly loads and utilizes the skill taxonomy JSON file
    3. **Enhanced Alias Matching**: Correctly maps all aliases to canonical skill names (e.g., "React.js" → "React")
    4. **Improved Normalization**: Uses taxonomy for accurate skill matching without cross-category contamination
    5. **Proper Hierarchy Display**: Shows meaningful skill relationships (e.g., "JavaScript → Node.js → Express.js")
    6. **Better Debug Output**: Added debug information to help identify matching issues
    7. **Preserved Original Names**: Skills are matched using normalized forms but displayed using original student skill names
    
    **🎯 Mind Map Legend:**
    - 🔵 **Center (Blue)**: Job Role
    - 🟢 **Green Branches**: Requirement Categories (Programming Language, Frameworks, etc.)
    - 🟡 **Yellow Nodes**: Parent Skills (e.g., Python, JavaScript)
    - 🔵 **Blue Nodes**: Standalone Skills (direct matches)
    - 🟣 **Purple Nodes**: Child Skills (e.g., Flask under Python)
    ...
    **Example Fix**: If your taxonomy has:
    ...
    Now when a student has "React.js" and job requires "react", they will match correctly and display beautifully in the mind map!
    ...
    **Enhanced Debugging Features:**
    ...
    - Interactive mind map shows visual relationships
    
    **Testing the Fix:**
    1. Upload your student CSV with "React.js" skills
    2. Create a job role requiring "react"
    3. Run evaluation - they should now match and display in the mind map!
    """)

st.markdown("---")
st.caption("Enhanced Student-Job Matcher with Interactive Mind Map Visualization 🚀 🧠 ✅")