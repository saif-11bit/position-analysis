...
class SimplifiedInternshipMatcher:
    ...
    def normalize_skills(self, skills: List[str]) -> List[str]:
        ...
        for skill in skills or []:
            ...
            # First, try to find canonical form from taxonomy
            canonical = self.skill_to_canonical.get(s)
            if canonical:
                normalized.add(canonical.lower())
                # Also add the original normalized form for backward compatibility
                normalized.add(s)
            else:
                # If not found in taxonomy, add as-is
                normalized.add(s)
    ...
    def calculate_skill_match_with_llm(
        ...
        for student_skill in student_skills:
            student_skill_norm = set(self.normalize_skills([student_skill]))
            if student_skill_norm.intersection(required_norm):
        ...
        for req_skill in required_skills:
            ...
            if req_expanded.intersection(student_norm):
                matched_requirements.add(req_skill.lower())
